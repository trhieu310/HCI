    ...
    <View style={styles.container}>
      <ImageBackground source={bg} style={styles.imgBg}>
        <View style={styles.viewText}>
          <Text style={[styles.text, styles.level]}>
            {level ? level : 'Beginner'}
          </Text>
          <Text style={[styles.text, styles.number]}>({number} từ)</Text>
          <Text style={[styles.text, styles.description]}>
            Để vượt qua cấp độ {level ? level : 'Beginner'} bạn cần học thêm{' '}
            {numberLeft} từ vựng mới.Hãy cố gắng để đạt được mục tiêu bạn nhé!{' '}
          </Text>
        </View>
      </ImageBackground>
    </View>
